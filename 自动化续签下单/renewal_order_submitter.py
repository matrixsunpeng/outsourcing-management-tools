...
class OutsourceContractSubmitter:
    ...
    def fill_signing_party(self, signing_party: str) -> bool:
        """
        在外包合同页面填写"签约方"字段。

        在 search_application 成功后（申请单已加载、弹窗已关闭）调用。
        定位签约方输入控件，清空并输入签约方名称，等待下拉并选中匹配项。

        Args:
            signing_party: 签约方名称（来自待办清单的"签约方"列）

        Returns:
            bool: 是否成功
        """
        logger.info(f"=== 填写签约方: {signing_party} ===")
        ...
        try:
            ...
            locate_result = self.page.evaluate("""() => {
                ...
                var labels = Array.from(document.querySelectorAll('label, td, th, span, div'))
                    .filter(function(el) {
                        return isVisible(el) && el.childNodes.length <= 5
                            && norm(el.textContent).indexOf('签约方') >= 0;
                    });
                ...
                for (var i = 0; i < labels.length; i++) {
                    var labelText = norm(labels[i].textContent);
                    if (!labelText || labelText.length > 15) continue;
                    var lr = labels[i].getBoundingClientRect();
                    for (var j = 0; j < inputs.length; j++) {
                        var inp = inputs[j];
                        var ir = inp.getBoundingClientRect();
            ...
            best_input = (locate_result.get("best") or {})
            if best_input.get("inputId"):
                party_input_id = best_input.get("inputId")
                party_input = self.page.locator(
                    f"input[id='{party_input_id}']"
                ).first

            if party_input is None or party_input.count() == 0:
                # 回退：尝试常见ID
                fallback_ids = [
                    "input[id='p_signing_party$text']",
                    "input[id='signingParty$text']",
                    "input[id='p_sign_party$text']",
                    "input[id='contractParty$text']",
                    "input[id*='signing']",
                    "input[id*='party']",
                    "input[id*='签约']",
                ]
                for sel in fallback_ids:
                    try:
                        loc = self.page.locator(sel).first
                        if loc.count() > 0:
                            party_input = loc
                            logger.info(f"[签约方] 回退选择器命中: {sel}")
                            break
                    except Exception:
                        continue

            if party_input is None or party_input.count() == 0:
                logger.error("[签约方] 未找到签约方输入框")
                return False

            # ── 步骤2：清空并输入签约方名称 ──
            party_input.click(force=True)
            self.page.wait_for_timeout(300)
            party_input.press("Control+a")
            party_input.press("Delete")
            party_input.type(signing_party, delay=60)
            logger.info(f"[签约方] 已输入: {signing_party}，等待下拉...")
            self.page.wait_for_timeout(1500)
            ...
            for drop_sel in dropdown_selectors:
                try:
                    ...
                    target_item = None
                    for _, item, text in visible_items:
                        if text == signing_party:
                            target_item = item
                            break
                    if target_item is None:
                        for _, item, text in visible_items:
                            if signing_party in text or text in signing_party:
                                target_item = item
                                break
                    if target_item is None:
                        target_item = visible_items[0][1]
                    ...
                    target_item.click()
                    clicked_text = (target_item.text_content() or "").strip()
                    logger.info(f"[签约方] 已点击下拉项: {clicked_text}")
                    self.page.wait_for_timeout(600)
                    clicked = True
                    break
                except Exception:
            ...
            if not clicked:
                try:
                    party_input.press("Enter")
                    self.page.wait_for_timeout(300)
                except Exception:
                    pass
                try:
                    party_input.press("Tab")
                    self.page.wait_for_timeout(300)
                except Exception:
                    pass
                current_val = party_input.input_value() or ""
                if current_val and (signing_party in current_val or current_val in signing_party):
                    logger.info(f"[签约方] 未出现下拉但输入框已有值: {current_val}")
                    return True
                logger.warning(f"[签约方] 未成功点击下拉选项，当前值: {current_val}")
                return False

            return True

        except Exception as e:
            logger.error(f"[签约方] 异常: {e}")
            return False
    ...
    def process_single_order(self, vendor_name: str, app_no: str,
                             work_location: str, not_renewing_df: Optional['pd.DataFrame'] = None,
                             signing_party: str = "") -> Tuple[bool, str]:
        """
        处理单条订单的完整流程
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"处理订单: {vendor_name} | {app_no} | 签约方: {signing_party}")
        logger.info(f"{'='*60}")
        ...
        try:
            ...
            # 2.5. 填写签约方（在申请单加载完成后、工作地点填充前）
            if signing_party:
                if not self.fill_signing_party(signing_party):
                    return False, f"签约方填写失败: {signing_party}"
            else:
                logger.warning("[签约方] signing_party 为空，跳过填写")

            # 3. 工作地点已取消自动填充（页面默认带出，无需手动填写）
